        ...
        <MenubarContent className="absolute -right-12 mt-3 min-w-[120px] rounded border py-2 dark:border-dark-400 dark:bg-dark-300">
          {themes.map((el) => {
            return (
              <MenubarItem
                key={el.value}
                className="flex items-center gap-4 px-2.5 py-2 dark:border-dark-400 dark:bg-dark-300"
                onClick={() => {
                  setMode(el.value);
                  if (el.value !== "system") {
              ...
              >
                ...
                />
                <p
                  className={`body-semibold text-light-500 ${mode === el.value ? "text-primary-500" : "text-dark100_light900"}`}
                >
                  {el.label}
                </p>
              </MenubarItem>
